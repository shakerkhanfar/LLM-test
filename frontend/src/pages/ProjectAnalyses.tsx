...
function healthBg(health: string | undefined): string {
  if (health === "Good") return T.successBg;
  if (health === "Fair") return T.warningBg;
  return T.errorBg;
}
    ...
    <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
      {evidence.map((ev, i) => (
        <div key={i} style={{
          ...
          {ev.quote && (
            <div style={{ color: T.textSecondary, fontStyle: "italic", lineHeight: 1.5 }}>
              "{ev.quote}"
            </div>
          )}
        </div>
    ...
    <div style={{
      ...
      >
        ...
        {av.comparedToVersion != null && (
          <div style={{ fontSize: 11, color: T.textMuted }}>compared to v{av.comparedToVersion}</div>
        )}
        ...
        <span style={{ color: T.border, fontSize: 10 }}>{expanded ? "▲" : "▼"}</span>
      </div>
        ...
        <div style={{ padding: "18px 18px 20px" }}>
          ...
          {av.comparison && (
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: T.link, marginBottom: 10 }}>
                Changes vs v{av.comparedToVersion}
              </div>
              {av.comparison.summary && (
                <div style={{ fontSize: 13, color: T.textSecondary, marginBottom: 12, lineHeight: 1.5 }}>
                  {av.comparison.summary}
                </div>
              )}
                    ...
                    <div key={key} style={{ flex: "1 1 220px" }}>
                      ...
                      {items.map((item, i) => (
                        <div key={i} style={{
                          ...
                          <div style={{ color: T.textSecondary, marginTop: 2 }}>{item.detail}</div>
                        </div>
                      ))}
                    </div>
                  );
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {a.priority_actions.map((action, i) => (
                  <div key={i} style={{
                    ...
                    padding: "10px 14px", background: T.cardAlt,
                    ...
                    <span style={{ fontSize: 13, color: T.text, lineHeight: 1.5 }}>{action}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              {a.critical_failures.map((f, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 6, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 11, color: T.textSecondary }}>{f.frequency}</span>
                    {f.prompt_location && (
                      <span style={{
                        fontSize: 10, padding: "1px 7px", borderRadius: 3,
                        background: "#1e3a5f44", color: T.link, border: "1px solid #1d4ed844",
                      }}>
                        {f.prompt_location}
                      </span>
                    )}
                  ...
                  {f.suggested_fix && (
                    <div style={{
                      ...
                      background: T.successBg, border: `1px solid ${T.successBg}`,
                      borderRadius: 4, color: "#22c55e", lineHeight: 1.5,
                    }}>
                      Fix: {f.suggested_fix}
                    </div>
                  )}
              ...
              {a.missing_edge_cases.map((ec, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#fcd34d", marginBottom: 4 }}>
                    {ec.scenario}
                    {ec.frequency && (
                      <span style={{ fontSize: 11, color: T.textSecondary, marginLeft: 10 }}>{ec.frequency}</span>
                    )}
                  </div>
                  {ec.impact && (
          ...
          {a?.prompt_issues?.length > 0 && (
            <Collapsible title={`Prompt & Node Issues (${a.prompt_issues.length})`} accent="#a855f7">
              {a.prompt_issues.map((pi, i) => (
                <div key={i} style={{
                  background: T.cardAlt, border: "1px solid #a855f733",
            ...
            <Collapsible title={`What's Working Well (${a.best_parts.length})`} accent="#22c55e">
              {a.best_parts.map((bp, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ fontSize: 13, color: T.textSecondary, lineHeight: 1.5 }}>{bp.detail}</div>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{
              ...
              ([val, label]) => (
                <button
                  ...
                  style={{
                    ...
                    background: dateFilterType === val ? T.primary : T.card,
                    color: dateFilterType === val ? "#fff" : T.textSecondary,
                    border: `1px solid ${dateFilterType === val ? "#3b82f6" : T.border}`,
                  }}
                >
                  {label}
                </button>
            ...
            <div>
              ...
              <input
                ...
                style={{
                  ...
                  color: T.text, padding: "6px 10px", fontSize: 12,
                }}
              />
            </div>
            <div style={{ color: T.textMuted, paddingBottom: 6 }}>→</div>
            <div>
              <label style={{ fontSize: 11, color: T.textMuted, display: "block", marginBottom: 4 }}>
                To
              </label>
        ...
        {/* Info text */}
        <p style={{ fontSize: 12, color: T.textSecondary, margin: "0 0 14px" }}>
        ...
        {dateFilterType && !from && !to && (
          <div style={{
            ...
            background: T.warningBg, border: "1px solid #78350f", color: "#fbbf24", fontSize: 12,
          }}>
        ...
        <button
          ...
          style={{
            background: running ? "#1e3a5f" : (!!dateFilterType && !from && !to) ? T.card : T.primary,
            color: (!!dateFilterType && !from && !to) ? T.textMuted : "#fff",
        ...
        <div>
          <div style={{
            ...
            <div style={{ fontSize: 13, color: T.textMuted }}>
              ...
              {selected.size > 0 && (
                <span style={{ color: T.link, marginLeft: 10 }}>
            ...
            {analyses.length >= 2 && (
              <button
                ...
                style={{
                  background: selected.size >= 2 ? "#4c1d95" : T.card,
                  color: selected.size >= 2 ? "#e9d5ff" : T.textMuted,
                  border: `1px solid ${selected.size >= 2 ? "#7c3aed" : T.border}`,
              ...
              {analyses.length >= 2 && (
                <div
                  ...
                  style={{
                    ...
                    border: `2px solid ${selected.has(av.id) ? "#3b82f6" : T.border}`,
                    background: selected.has(av.id) ? T.primary : "transparent",
