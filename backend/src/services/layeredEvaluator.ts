...
- INTENT ALREADY STATED — NOT A VIOLATION: If the node's instructions say "ask [question]" (e.g. "ask if they want to book, cancel, or reschedule") but the USER already stated their intent before or while the agent reached this node, the agent skipping that question and moving directly to the relevant flow is ADAPTIVE and CORRECT — do NOT list it as a violation. Example: user says "I want to book a dental appointment" → agent skips the intent question and moves to collecting patient data → this is correct. Only flag it as violated if the user never stated their intent AND the agent still didn't ask.
...
${outcomeResult ? (() => {
  ...
  const lines = Object.entries(outcomeResult)
    ...
    .join("\n");
  return lines ? `\nSYSTEM-RECORDED OUTCOME (recorded by the platform after the call):\n${lines}\nNOTE ON INTERPRETING OUTCOME: If objective_met = "no" but the transcript shows the agent was actively serving the user when the call ended (e.g., presenting appointment options, confirming details), that means the CALLER abandoned or hung up — not that the agent failed. In such cases, quality_score should reflect the agent's performance up to that point (typically 7-8 for good service interrupted by caller). Only score low if the agent itself caused the failure.\nSYSTEM LIMITATION RULE: If the agent checked the system (called tools) and the system returned NO availability (no appointments, no slots, no matching records), and the agent correctly communicated this to the caller and offered alternatives (different dates, different doctors, main hospital number) — that is NOT an agent failure. The agent cannot create availability that doesn't exist. In such cases: quality_score should be 8-9 (agent handled an impossible situation well), objective_achieved should be null (system limitation, not agent failure). Only penalize if the agent failed to check the system, gave wrong information, or didn't offer alternatives.` : "";
})() : ""}
