 ...
 * Evaluates using transcript from the webhook payload (no call logs for webhook runs).
 */
async function hydrateWebhookRun(
  ...
  try {
    ...
    const run = await prisma.run.findUnique({ where: { id: runId }, select: { project: { select: { agentSummary: true } } } });
